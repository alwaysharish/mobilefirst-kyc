pipeline {
    ...
    environment {
        ...
        ECR_REPO = "${ECR_ACCOUNT}.dkr.ecr.${AWS_REGION}.amazonaws.com/mobilefirst/kyc"
        ...
        stage('Clone App Repository') {
            steps {
                git branch: "${BRANCH_NAME}",
                    ...
                    url: "https://github.com/alwaysharish/mobilefirst-kyc.git"
            }
        }
            ...
            steps {
                dir('mobilefirst-manifests') {
                    ...
                    sh """
                        ...
                        sed -i 's|image: ${ECR_ACCOUNT}.dkr.ecr.${AWS_REGION}.amazonaws.com/mobilefirst:.*|image: ${ECR_PUBLISH_IMAGE_TAG}|g' kyc/deployment-svc.yaml
                        echo "Updated file preview:"
                        cat kyc/deployment-svc.yaml | grep image
                        git config user.email "jenkins@mobilefirst.com"
                        git config user.name "jenkins"
                        git add kyc/deployment-svc.yaml
                        git commit -m "Update KYC image to ${ECR_PUBLISH_IMAGE_TAG}" || echo "No changes to commit"
